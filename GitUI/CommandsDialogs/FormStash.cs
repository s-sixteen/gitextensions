    ...
    public sealed partial class FormStash : GitModuleForm
    {
        ...
        private void StashedSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (WaitCursorScope.Enter())
            {
                ...
                else if (gitStash == _currentWorkingDirStashItem)
                {
                    // current working directory
                    View.ViewChangesAsync(ObjectId.IndexId, ObjectId.WorkTreeId, stashedItem);
                }
                else
                {
                    var first = Module.RevParse(gitStash.Name + "^");
                    var selected = Module.RevParse(gitStash.Name);
                    View.ViewChangesAsync(first, selected, stashedItem);
                }
            }
        }
